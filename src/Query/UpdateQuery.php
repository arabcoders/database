...
final class UpdateQuery implements QueryInterface, CacheableQueryInterface
{
    use Macroable;
    ...
    /**
     * @var array<int,array{type:string,table:string|QueryInterface,alias:null|string,on:Condition|string|null,subquery:bool}>
     */
    private array $joins = [];
    ...
    /**
     * @var array<int,array{type:string,column:string,direction:null|string}>
     */
    ...
    /**
     * Attach a common table expression used by this update query.
     *
    ...
    /**
     * Join a subquery in the UPDATE statement.
     *
    ...
    {
        $this->values[$column] = new RawExpression($expression);
    ...
    /**
     * Compile the update query into SQL and bound parameters.
     *
     * @param DialectInterface $dialect SQL dialect used to render identifiers and clauses.
     * @return array{sql:string,params:array<string,mixed>}
     * @throws RuntimeException If no update values are set.
     * @throws RuntimeException If no WHERE condition is set.
     * @throws RuntimeException If the dialect does not support a requested feature.
     */
    public function toSql(DialectInterface $dialect): array
    {
        if (empty($this->values)) {
            throw new RuntimeException('Update values are required.');
        }
        ...
        foreach ($this->values as $column => $value) {
            if ($value instanceof RawExpression) {
                $parts[] = Identifier::quote($dialect, $column) . ' = ' . $value->sql();
                continue;
            }
            $parts[] = Identifier::quote($dialect, $column) . ' = ' . $params->add($value);
        }

        $from = $this->renderFrom($dialect, $params);

        $sql = 'UPDATE ' . Identifier::quoteWithAlias($dialect, $this->table, $this->alias);
        $sql .= $this->renderJoins($dialect, $params);
        $sql .= $from['sql'];
        $sql .= ' SET ' . implode(', ', $parts);

        $whereParts = $from['conditions'];
        $whereParts[] = $this->where->toSql($dialect, $params);
        $sql .= ' WHERE ' . $this->renderWhereParts($whereParts);
    ...
    private function renderJoins(DialectInterface $dialect, ParameterBag $params): string
    {
        ...
        if ('sqlite' === $dialectName) {
            throw new RuntimeException('Update joins are not supported for ' . $dialect->name() . '.');
        }

        if ('mysql' === $dialectName) {
            return $this->renderMysqlJoins($dialect, $params);
        }

        if ('pgsql' === $dialectName) {
            if ($this->hasNonInnerJoin()) {
                throw new RuntimeException('Postgres update joins only support INNER joins.');
            }
            return '';
        }
    ...
    private function renderMysqlJoins(DialectInterface $dialect, ParameterBag $params): string
    {
        $sql = '';
    ...
    private function renderFrom(DialectInterface $dialect, ParameterBag $params): array
    {
        ...
        return [
            'sql' => ' FROM ' . implode(', ', $targets),
            'conditions' => $conditions,
        ];
    }

    private function hasNonInnerJoin(): bool
    {
        foreach ($this->joins as $join) {
            if ('INNER' === $join['type']) {
                continue;
